'''
Author: fengsc
Date: 2022-09-12 20:18:56
LastEditTime: 2022-09-12 23:08:23
'''
# coding=utf-8
import cv2
import numpy as np
...
class YOLOV5_ONNX(object):
    ...
    def nms(self, prediction, conf_thres=0.1, iou_thres=0.6, agnostic=False):
        ...
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            x = torch.cat((torch.tensor(box), conf, j.float()), 1)[
    ...
    def infer(self, img_path):
        ...
        anchor = np.array(anchor_list).astype(float).reshape(3, -1, 2)

        area = img_size[0] * img_size[1]
        size = [int(area / stride[0] ** 2), int(area /
                                                stride[1] ** 2), int(area / stride[2] ** 2)]
        ...
        pred = self.onnx_session.run(
            output_names=self.output_name, input_feed=input_feed)
        print(type(pred[0]))
        print(pred[0].shape)
        # 提取出特征
        y = []
        # y.append(torch.tensor(pred[0]).sigmoid())
        # y.append(torch.tensor(pred[1]).sigmoid())
        # y.append(torch.tensor(pred[2]).sigmoid())
        y.append(torch.tensor(
            pred[0].reshape(-1, size[0]*3, 5+class_num)).sigmoid())
        y.append(torch.tensor(
            pred[1].reshape(-1, size[1]*3, 5+class_num)).sigmoid())
        y.append(torch.tensor(
            pred[2].reshape(-1, size[2]*3, 5+class_num)).sigmoid())
        ...
        for i in range(3):
            ...
            for j in range(3):
                dst_xy.append(
                    (xy[:, j * size[i]:(j + 1) * size[i], :] + torch.tensor(grid[i])) * stride[i])
                dst_wh.append(wh[:, j * size[i]:(j + 1) *
                              size[i], :] * anchor[i][j])
            src[..., 0:2] = torch.from_numpy(np.concatenate(
                (dst_xy[0], dst_xy[1], dst_xy[2]), axis=1))
            src[..., 2:4] = torch.from_numpy(np.concatenate(
        ...
        for det in results:  # detections per image
            if det is not None and len(det):
                det[:, :4] = self.scale_coords(
                    img_shape, det[:, :4], src_size).round()
    ...
    def draw(self, img, boxinfo):
        ...
        for *xyxy, conf, cls in boxinfo:
            ...
            print('xyxy: ', xyxy)
            self.plot_one_box(xyxy, img, label=label,
...
if __name__ == "__main__":
    model = YOLOV5_ONNX(
        onnx_path="yolov5/runs/train/robo4_epoch150_s/weights/best.onnx")
    model.infer(img_path="/home/fengsc/Desktop/maksssksksss0.png")
